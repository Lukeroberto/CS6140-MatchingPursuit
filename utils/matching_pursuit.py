...
def convolutionalMatchingPursuit(image, features, k_matches, verbose=False):
    ...
    for _ in pbar:
        ...
        for i in range((2*f_size-1)**2):
            dx, dy = i // (2*f_size-1)-(f_size-1), i % (2*f_size-1)-(f_size-1)
            _x,_y = x+dx, y+dy
            if _x<0 or _x > (e.shape[0]-f_size-1) or _y < 0 or _y > (e.shape[0]-f_size-1):
                continue
